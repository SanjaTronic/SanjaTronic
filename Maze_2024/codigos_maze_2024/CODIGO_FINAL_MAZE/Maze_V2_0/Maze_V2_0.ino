// alterar pinos 22 e 24 (trigger e echo) do ultrasonico 3 para outras portas digitais, pois está dando conflito entre os pinos do ultrasonico 3 e o sensor de cor 

#include <NewPing.h> // Biblioteca para uso do sensor ultrassônico
#include <Wire.h>    // Biblioteca para comunicação I2C
#include <Servo.h>   // Biblioteca para uso do servo motor
#include <SFE_MMA8452Q.h> // bilbioteca para o giroscópio
...
int CurvaLeftSpeed = 132; // Velocidade do motor direito para curva à direita
...
void setup() {
  ...
  meuServo.attach(3);  // Pino digital 9 para o sinal do servo
...
void loop() {
 ...
 if (Serial.available()) {
    ...
    int delayTime = 0;
    switch(receivedByte) {
      case 1: //letra H detectada
        Parar();
        piscaLed();
        delayTime = 310;  // Tempo para 2 kits
        break;
      case 2: // Letra S detectada
        Parar();
        piscaLed();
        delayTime = 155;  // Tempo para 1 kit
        break;
      case 3: // letra U detectada
        Parar();
        piscaLed();
        break;
      case 4: // Cor vermelha detectada
        Parar();
        piscaLed();
        delayTime = 310;  // Tempo para 2 kits
        break;
      case 5: // Cor amarela detectada
        Parar();
        piscaLed();
        delayTime = 155;  // Tempo para 1 kit
        break;
      case 6: // Cor verde detectada
        Parar();
        piscaLed();
        break;
    }
  ...
  if (isBlue(red, green, blue)) {
    ...
    paraTras();
  } if (isSilver(red, green, blue)) {
    Parar();
    delay(500);
  }
    //else if (acelerometro.available()) {
