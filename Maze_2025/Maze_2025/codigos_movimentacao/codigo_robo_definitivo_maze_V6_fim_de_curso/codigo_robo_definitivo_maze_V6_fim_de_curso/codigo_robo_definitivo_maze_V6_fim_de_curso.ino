/*
========================================================================= ROBÔ DEFINITIVO MAZE V6.0 ========================================================================================
Autor: Murilo
Data da criação: 21/07/2025
Data da última modificação: 17/09/2025
...
// Definição de pinos dos botões fim de curso
#define pinoBotaoEsquerdo 52
#define pinoBotaoDireito 47
...
const int grausGiroE45 = -45;
const int grausGiroD45 = 45;
...
// Variáveis dos botões fim de curso
bool botaoEsquerdoPressionado = 0;
bool botaoDireitoPressionado = 0;
...
void setup() 
{
  ...
  // Fim de curso
  pinMode(pinoBotaoEsquerdo, INPUT_PULLUP);
  pinMode(pinoBotaoDireito, INPUT_PULLUP);
...
void fimDeCurso()
{
  bool estadoFimDeCursoEsquerdo = digitalRead(pinoBotaoEsquerdo);
  bool estadoFimDeCursoDireito = digitalRead(pinoBotaoDireito);

  if(estadoFimDeCursoEsquerdo == LOW && estadoFimDeCursoDireito == LOW)
  {
    botaoEsquerdoPressionado = true;
    botaoDireitoPressionado = true;
    Serial.println("Ambos os fins de curso foram pressionados");
    paraTras();
    delay(300);
    virarParaMaiorLadoLaser();
  }
  else if (estadoFimDeCursoEsquerdo == LOW) // Se o fim de curso esquerdo for pressionado
  {
    botaoEsquerdoPressionado = true;

    Serial.print("o botão esquerdo foi pressionado!: ");
    Serial.println(botaoEsquerdoPressionado);
    paraTras();
    delay(300);
    virarDireita45G();
  }
  else if(estadoFimDeCursoDireito == LOW) // Se o fim de curso direito for pressionado
  {
    botaoDireitoPressionado = true;

    Serial.print("o botão direito foi pressionado!: ");    
    Serial.println(botaoDireitoPressionado);
    paraTras();
    delay(300);
    virarEsquerda45G();
  }
...
void virarEsquerda45G()
{
  ...
  {
    mpu_loop(); // Executa a lógica do giroscópio
    float anguloE45 = getAngleZ();

    // Verifica a angulação do giorscópio
    if(anguloE45 < grausGiroE45) // Caso o giroscópio ultrapasse 90 graus negativo (finaliza uma curva a esquerda)
    {
      pararMotores();
      delay(50);
      break; // Sai do loop do while
...
void virarDireita45G()
{
  ...
  {
    mpu_loop(); // Executa a lógica do giroscópio
    float anguloD45 = getAngleZ();

    // Verifica a angulação do giroscópio
    if(anguloD45 > grausGiroD45) // Caso o giroscópio ultrapasse 90 graus (finaliza uma curva a direita)
    {
      pararMotores();
      delay(50);
      break; // Sai do loop do while
