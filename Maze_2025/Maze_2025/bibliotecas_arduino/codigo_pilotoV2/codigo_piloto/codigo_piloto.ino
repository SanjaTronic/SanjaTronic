...
//#include Servo.h // Biblioteca para uso do servo motor do dispenser
...
const int tempoDelayLeituraLasers = 100;
...
const int diferencaLimite = 15;
bool distanciaInicialRegistrada = false;
...
void loop() 
{
  ...
  {
    // Inicia o movimento se frente está livre e ainda não está andando
    if(frente < 0)
    {
      s_println("erro ao medir a frente.....");
      paraFrente();
      delay(1300);
      pararMotores();
      delay(2000);
    }
    else if ((frente > 7 && !emMovimento))
    {
      paraFrente();

      if (!distanciaInicialRegistrada)
      {
        distanciaInicial = frente;
        distanciaInicialRegistrada = true;
        emMovimento = true;
        s_println("Distância inicial registrada.");
      }
    }

    // Verifica se já andou os 20 cm
    if (emMovimento)
    {
      distanciaAtual = frente;

      if (distanciaInicial - distanciaAtual >= diferencaLimite)
      {
        pararMotores();
        emMovimento = false;
        distanciaInicialRegistrada = false;
        s_println("Andou 20 cm e parou.");
        delay(500);
      }
    }

    // Se encontrou obstáculo à frente
    if (frente <= 7 && frente > 0)
    {
      pararMotores();
      emMovimento = false;
      distanciaInicialRegistrada = false;

      if ((esquerda > direita && frente > 0) || (esquerda * -1) > direita)
      {
        s_println("Virando à esquerda...");
        paraTras();
        delay(tempoDelayRecuo);
        virarEsquerda90G();
      }
      else if ((direita > esquerda && frente > 0) || (direita * -1) > esquerda)
      {
        s_println("Virando à direita...");
        paraTras();
        delay(tempoDelayRecuo);
        virarDireita90G();
      }
